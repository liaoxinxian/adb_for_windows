...
static char *product_file(const char *extra)
{
	...
	sprintf(x, "%s" OS_PATH_SEPARATOR_STR "%s", gProductOutPath, extra);
	return x;
}
...
{
	if (serial) {
		sprintf(buffer,"host-serial:%s:%s", serial, command);
	} else {
...
static int top_works(const char *top)
{
	if (top != NULL && adb_is_absolute_host_path(top)) {
		...
		return access(path_buf, 0) == 0;
	}
	return 0;
}

static char *find_top_from(const char *indir, char path_buf[MAX_PATH])
{
...
static char *find_top(char path_buf[MAX_PATH])
{
...
top:
	...
	if(!strcmp(argv[0], "connect")) {
		...
		sprintf(buf, "host:connect:%s", argv[1]);
		tmp = adb_query(buf);
		if(tmp) {
			printf("%s\n", tmp);
			return 0;
	...
	if(!strcmp(argv[0], "disconnect")) {
		...
		if (argc == 2) {
			sprintf(buf,"host:disconnect:%s", argv[1]);
		} else {
			sprintf(buf, "host:disconnect:");
		}
		tmp = adb_query(buf);
		if(tmp) {
			printf("%s\n", tmp);
	...
	if(!strcmp(argv[0], "remount") || !strcmp(argv[0], "reboot")
		...
		if (!strcmp(argv[0], "reboot-bootloader"))
			sprintf(command, "reboot:bootloader");
		else if (argc > 1)
			sprintf(command, "%s:%s", argv[0], argv[1]);
		else
	...
	if(!strcmp(argv[0], "forward")) {
		...
		if (serial) {
			sprintf(buf,"host-serial:%s:forward:%s;%s",serial, argv[1], argv[2]);
		} else if (ttype == kTransportUsb) {
			sprintf(buf, "host-usb:forward:%s;%s", argv[1], argv[2]);
		} else if (ttype == kTransportLocal) {
			sprintf(buf, "host-local:forward:%s;%s", argv[1], argv[2]);
		} else {
			sprintf(buf, "host:forward:%s;%s", argv[1], argv[2]);
		}
		if(adb_command(buf)) {
			fprintf(stderr,"error: %s\n", adb_error());
			return 1;
...
int install_app(transport_type transport, char* serial, int argc, char** argv)
{
	...
	char apk_dest[MAX_PATH];
	char verification_dest[MAX_PATH];
	char* apk_file;
	...
	sprintf(apk_dest, where, get_basename(apk_file));
	if (verification_file != NULL) {
		sprintf(verification_dest, where, get_basename(verification_file));
