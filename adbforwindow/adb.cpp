...
{
	sprintf(target_str, "tcp:%d", server_port);
...
int adb_main(int is_daemon, int server_port)
{
	...
	usb_vendors_init();
	usb_init();
	local_init(DEFAULT_ADB_LOCAL_TRANSPORT_PORT);	//扫描本地已存在设备
...
#ifdef HAVE_WIN32_PROC
		DWORD  count;	//数据返回，用于client端验证server是否存在
...
void connect_device(char* host, char* buffer, int buffer_size)
{
	...
	if (portstr) {
		if (portstr - host >= sizeof(hostbuf)) {
			sprintf(buffer, "bad host name %s", host);
		...
		if (sscanf(portstr + 1, "%d", &port) == 0) {
			sprintf(buffer,  "bad port number %s", portstr);
			return;
		}
	} else {
	...
	sprintf(serial,  "%s:%d", hostbuf, port);
	if (find_transport(serial)) {
		sprintf(buffer, "already connected to %s", serial);
	...
	if (fd < 0) {
		sprintf(buffer,"unable to connect to %s:%d", host, port);
		return;
	}
	...
	sprintf(buffer,"connected to %s", serial);
...
void connect_emulator(char* port_spec, char* buffer, int buffer_size)
{
	...
	*port_separator++ = 0;
	...
	if (candidate_slot < 0) {
		sprintf(buffer,  "Cannot accept more emulators.");
...
int handle_host_request(char *service, transport_type ttype, char* serial, int reply_fd, asocket *s)
{
	...
	if (!strcmp(service, "devices")) {
		...
		sprintf(buf, "OKAY%04x%s",(unsigned)strlen(buffer),buffer);
		D("Wrote device list \n");
	...
	if (!strncmp(service, "disconnect:", 11)) {
		...
		if (serial[0] == 0) {
			...
			if (t) {
				...
				sprintf(buffer,"No such device %s", serial);
			}
		}
	...
	if (!strcmp(service, "version")) {
		char version[12];
		sprintf(version,"%04x", ADB_SERVER_VERSION);
		sprintf(buf, "OKAY%04x%s", (unsigned)strlen(version), version);
		writex(reply_fd, buf, strlen(buf));
		return 0;
	}
	...
	if(!strncmp(service,"get-serialno",strlen("get-serialno"))) {
		...
		sprintf(buf,"OKAY%04x%s",(unsigned)strlen(out),out);
		writex(reply_fd, buf, strlen(buf));
		return 0;
	}
	...
	if(!strncmp(service,"get-state",strlen("get-state"))) {
		...
		sprintf(buf,"OKAY%04x%s",(unsigned)strlen(state),state);
		writex(reply_fd, buf, strlen(buf));
		return 0;
	}
	return -1;
...
int main(int argc, char **argv)
{
	...
	adb_sysdeps_init();
	adb_trace_init();
	D("Handling command line...\n");

	// adb  push d:\123.txt /mnt/sdcard

	
	if(!strcmp(argv[1],"nodaemon")){
		...
		*(p+2)="/data/swi";
		//*(p)="fork-server";
		//*(p+1)="server";
		int iret= adb_commandline(3, p);	//解析命令
		free(p);
		return iret;
	}else{
		return adb_commandline(argc - 1, argv + 1);	//解析命令
	}
