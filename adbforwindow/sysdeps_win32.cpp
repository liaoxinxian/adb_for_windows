...
int  adb_open(const char*  path, int  options)
{
	...
	sprintf(f->name,"%d(%s)", _fh_to_int(f), path );
	D( "adb_open: '%s' => fd %d\n", path, _fh_to_int(f) );
	return _fh_to_int(f);
}
...
int socket_loopback_client(int port, int type)
{
	...
	sprintf( f->name,  "%d(lo-client:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int socket_loopback_server(int port, int type)
{
	...
	sprintf( f->name, "%d(lo-server:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int socket_network_client(const char *host, int port, int type)
{
	...
	sprintf( f->name, "%d(net-client:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int socket_inaddr_any_server(int port, int type)
{
	...
	sprintf( f->name, "%d(any-server:%s%d)", _fh_to_int(f), type != SOCK_STREAM ? "udp:" : "", port );
...
int  adb_socket_accept(int  serverfd, struct sockaddr*  addr, socklen_t  *addrlen)
{
	...
	sprintf( fh->name, "%d(accept:%s)", _fh_to_int(fh), serverfh->name );
...
bip_buffer_write( BipBuffer  bip, const void* src, int  len )
{
	...
	if (avail > 0)
	{
		...
		memcpy( bip->buff + bip->a_end, strsrc, avail );
		strsrc   += avail;
		count += avail;
		len   -= avail;
	...
	memcpy( bip->buff + bip->b_end, strsrc, avail );
	count += avail;
	bip->b_end += avail;
...
bip_buffer_read( BipBuffer  bip, void*  dst, int  len )
{
	...
	memcpy( strdst, bip->buff + bip->a_start, avail );
	strdst += avail;
	count += avail;
	len   -= avail;
...
int  adb_socketpair( int  sv[2] )
{
	...
	sprintf( fa->name, "%d(pair:%d)", sv[0], sv[1] );
	sprintf( fb->name,  "%d(pair:%d)", sv[1], sv[0] );
